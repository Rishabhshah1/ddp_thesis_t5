...
from scripts.data_utils import (AccuracyMetrics, CalMSELoss, LineByLineTextDataset,
                        T5ChemTasks, TaskPrefixDataset, TaskSettings,
                        data_collator)
from scripts.model import T5ForProperty
from scripts.mol_tokenizers import (AtomTokenizer, MolTokenizer, SelfiesTokenizer,
                            SimpleTokenizer)
from scripts.trainer import EarlyStopTrainer

# from transformers import PreTrainedTokenizerFast
# tokenizer_ours = PreTrainedTokenizerFast(tokenizer_file="/scratch/scratch1/rishabh/t5chem/tokenizer_bpe.json")
# if tokenizer_ours.pad_token is None:
#     tokenizer_ours.add_special_tokens({'pad_token': '[PAD]'})
...
def add_args(parser):
    ...
    parser.add_argument(
        "--tokenizer",
        default='',
        help="Tokenizer to use. ('simple', 'atom', 'selfies', 'ours')",
    )
    parser.add_argument(
        "--random_seed",
        default=8570,
...
def train(args):
    ...
    if args.pretrain: # retrieve information from pretrained model
        ...
        assert args.tokenizer in ('simple', 'atom', 'selfies', 'ours'), \
